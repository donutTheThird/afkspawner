...
function setupEventHandlers() {
  bot.once('spawn', async () => {
    ...
    setInterval(checkForSpawners, 30000);
    setInterval(checkInventoryForBones, 45000);
    setInterval(mainActivityLoop, CHECK_INTERVAL);
  });
...
function processQueue() {
  if (botState.processingQueue || botState.isBusy || botState.actionQueue.length === 0) {
    return;
  }
...
// --- Chest Handling ---
async function emptyInventoryToChest() {
...
// Update the main activity loop to remove the chest check
setInterval(async () => {
  ...
  if (nearbyPlayer) {
    await findAndBreakTargets(nearbyPlayer);
  }
  // Removed the chest check here since it's now handled immediately after breaking
}, CHECK_INTERVAL);
...
function mainActivityLoop() {
  ...
  if (nearbyPlayer) {
    findAndBreakTargets(nearbyPlayer);
  }
  
  // Queue chest emptying if we have spawners
  const spawnersInInventory = bot.inventory.items().filter(i => i.name === TARGET_ITEM).length;
  if (spawnersInInventory > 0) {
    addToQueue({ type: 'chest' });
  }
